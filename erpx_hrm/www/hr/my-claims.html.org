          ...
          <div id="card-stats" class="card" style="border-radius: 10px!important;">
            <div class="card-content">
              ...
              <div class="right set-margin btn-left" style="margin-left: -20px !important">
                ...
                <a href="#create-claims.modal" class="btn modal-trigger waves-effect waves-light create-btn btn-left">
                Create Claim</a>
              </div>
              <br><br>
              <div class="row">
                    ...
                    <table class="table table-striped dataTable display" style="font-size: 14px;" id="table_id">
                        <thead>
                            <tr style="border-bottom: 1px solid lightgray !important;font-size: 14px;background-color: #f9f9f9; width: 100%!important;" >
                                ...
                                <th style="border-right:none!important;">Period</th>
                                <th style="border-right:none!important;">Amount</th>
                                <th style="border-right:none!important;">Last Updated</th>
                                <th style="border-right:none!important;">Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            {% set exp_list = frappe.get_all("Expense Claim", fields=['*']) %}
                            {% for i_exp_list in exp_list %}
                                <tr>
                                    <td><a style="color: black" href="/claims/claim-title1.html">{{i_exp_list.name}}</a></td>
                                    <td>{{ i_exp_list.posting_date.strftime(" %d %b %Y ")  }} - {{ i_exp_list.posting_date.strftime(" %d %b %Y ")  }}</td>
                                    <td>MYR {{i_exp_list.total_claimed_amount}}</td>
                                    <td>{{ i_exp_list.modified.date()  }}</td>
                                    {% set class_color = '' %}
                                    {% if(i_exp_list.status == 'Unpaid') %}
                                        {% set class_color = 'p-pending' %}
                                    {% elif(i_exp_list.status == 'Draft') %}
                                        {% set class_color = 'p-pending' %}
                                    {% elif(i_exp_list.status == 'Cancelled') %}
                                        {% set class_color = 'p-declined' %}
                                    {% elif(i_exp_list.status == 'Rejected') %}
                                        {% set class_color = 'p-declined' %}
                                    ...
                                    <td><p class="{{class_color}}">{{i_exp_list.status}}</p></td>
                                </tr>
                            {% endfor %}
                        </tbody>
                    </table>
                            ...
                            <div id="create-claims.modal" class="modal modal-fixed-footer" style="padding-right: 50px;">
                                <div class="modal-content">
                                    <p class="edit-title">Create Claim</p>
                                    <form class="col s12">
                                        <div class="row">
                                ...
                                <div class="modal-footer">
                                    ...
                                    <a href="#" class="right blue-text btn-flat"
                                       onclick="$('#create-claims\\.modal').modal('close')">Cancel</a>
                                </div>
                            </div>			
